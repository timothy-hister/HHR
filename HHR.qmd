...
# returns a dataframe of entrants
...
simulate_one_year = function(df, logit, entrants, h) {
  df = df |> filter(!retired)
    df$entered = F
    retired_idx = calc_retirements(df, logit_spec)
    df$retired[retired_idx] = T
    df$age = df$age + 1
    entrants = calc_entrants(entrants_spec, h)
    df = bind_rows(df, entrants)
...
logits = actuals |>
  crossing(models) |>
  rename(actuals = data) |>
  mutate(data = list(calc_logit(first(actuals), equation))) |>
  select(-actuals, -equation)
...
#forecasts = actuals
actuals |>
  inner_join(logits, by='spec_id') |>
  inner_join(entrants, by='spec_id') |>
  apply(1, function(row) {
  browser()
  logit_spec = filter(logits, spec_id == spec) |> pull(data) |> first()
  #df_init = actuals |>
  df_init = row$data.x |>
      #filter(spec_id == spec) |>
      #pull(data) |>
  ...
  entrants_spec = entrants_all |> filter(spec_id == spec) |> pull(data) |> first()
  
  accumulate(1:params$num_years_to_forecast, function(df, h) {
    browser()
    simulate_one_year(df, logit_spec, entrants_spec, h)
  }, .init = df_init) |>
    bind_rows() |>
